...
int esp_mbedtls_setup_tx_buffer(mbedtls_ssl_context *ssl)
{
    ...
    *(ssl->out_buf - SSL_BUF_HEAD_OFFSET_SIZE) = ESP_MBEDTLS_SSL_BUF_NO_CACHED;
...
int esp_mbedtls_add_tx_buffer(mbedtls_ssl_context *ssl, size_t buffer_len)
{
    ...
    if (ssl->out_buf) {
        if (*(ssl->out_buf - SSL_BUF_HEAD_OFFSET_SIZE) == ESP_MBEDTLS_SSL_BUF_CACHED) {
...
int esp_mbedtls_free_tx_buffer(mbedtls_ssl_context *ssl)
{
    ...
    if (!ssl->out_buf || (ssl->out_buf && (*(ssl->out_buf - SSL_BUF_HEAD_OFFSET_SIZE) == ESP_MBEDTLS_SSL_BUF_NO_CACHED))) {
        ret = 0;
        goto exit;
    }
...
int esp_mbedtls_add_rx_buffer(mbedtls_ssl_context *ssl)
{
    ...
    if (ssl->in_buf) {
        if (*(ssl->in_buf - SSL_BUF_HEAD_OFFSET_SIZE) == ESP_MBEDTLS_SSL_BUF_CACHED) {
...
int esp_mbedtls_free_rx_buffer(mbedtls_ssl_context *ssl)
{
    ...
    if (!ssl->in_buf || (ssl->in_hslen && (ssl->in_hslen < ssl->in_msglen)) ||
        (ssl->in_buf && (*(ssl->in_buf - SSL_BUF_HEAD_OFFSET_SIZE) == ESP_MBEDTLS_SSL_BUF_NO_CACHED))) {
        ret = 0;
        goto exit;
    }
    ...
    init_rx_buffer(ssl, esp_buf->buf);
    *(ssl->in_buf - SSL_BUF_HEAD_OFFSET_SIZE) = ESP_MBEDTLS_SSL_BUF_NO_CACHED;
exit:
    ESP_LOGV(TAG, "<-- free rx");
